...
description: Choose a compatible Loader, add the DownloadIt nightly XPI to Firefox, restart the browser, and upgrade an existing installation.
sidebar:
  order: 1
...
All supported Firefox channels can use the customized [`userChrome.js-Loader`](https://github.com/benzBrake/userChrome.js-Loader/releases). For Firefox Developer Edition and Nightly, you may use [`Bootstrap Loader`](https://github.com/benzBrake/BootstrapLoader/) instead.
...
:::caution
`userChrome.js-Loader` changes Firefox's application and profile files. Use a dedicated profile or back up the affected files before installing it.
:::
...
2. Download [`BootstrapLoader.xpi`](https://github.com/benzBrake/BootstrapLoader/releases/download/latest/BootstrapLoader.xpi) from the latest release assets.
...
:::caution
These preferences reduce Firefox's add-on signature and WebExtension Experiment restrictions. Use Bootstrap Loader only in a dedicated Developer Edition or Nightly profile.
:::
